...
def lk(ident):
    ...
    for r in rowslog:
        ...
        resid = r[5]
    cur.execute(
        "SELECT  * from AUTO,ADVERTISEMENT WHERE ID_OF_SELLER =  %s AND AUTO.VIN_NUMBER_OF_AUTO = ADVERTISEMENT.VIN_NUMBER_OF_AUTO",
        [ident])
    rowslk = cur.fetchall()
...
def register():
    if request.method == 'POST' and request.form["login"].replace(" ", "") != '' and request.form["password"].replace(
            " ", "") != '' and request.form["fio"].replace(" ", "") != '' and request.form["telephone"].replace(" ",
                                                                                                                "") != '' and \
            request.form["residence"].replace(" ", "") != '':
        cur.execute("SELECT * from SELLER")
        rowslog = cur.fetchall()
        for r in rowslog:
            id = (r[0])
...
def lkredactor(ident):
    ...
    for r in rowslog:
        ...
        tnumber = r[4]
        resid = r[5]
    if request.method == 'POST' and request.form['submit_1'] == "Изменить" and (
            request.form["fio"] != '' or request.form["telephone"] != '' or request.form["residence"] != 'Не выбрано'):
        ...
        if tnumber != request.form["telephone"] and request.form["telephone"] != '':
            cur.execute("UPDATE SELLER SET TELEPHONE_NUMBER =  %s WHERE ID_OF_SELLER = %s",
                        (request.form["telephone"], ident))
            con.commit()
...
def avtopluss(ident, identm):
    ...
    rowss = cur.fetchall()
    if request.method == 'POST' and request.form["vin"] != '' and request.form["amount"] != '' and request.form[
        "col"] != '' and request.form["year"] != '' and request.form["mile"] != '' and request.form["mile"] != '' and \
            request.form["state"] != '' and request.form["comm"] != '':
...
def advred1(vinn, ident):
    ...
    if request.method == 'POST' and (
            request.form["num"] != '' or request.form["com"] != '' or request.form["amon"] != '' or request.form[
        "colo"] != '' or request.form["yea"] != '' or request.form["mile"] != ''):
        if state != request.form["num"] and request.form["num"] != '':
            cur.execute("UPDATE ADVERTISEMENT SET STATE_NUMBER =  %s WHERE VIN_NUMBER_OF_AUTO = %s",
                        (request.form["num"], vinn))
            con.commit()
        if comm != request.form["com"] and request.form["com"] != '':
            cur.execute("UPDATE ADVERTISEMENT SET SELLERS_COMMENT =  %s WHERE VIN_NUMBER_OF_AUTO = %s",
                        (request.form["com"], vinn))
            con.commit()
        if amo != request.form["amon"] and request.form["amon"] != '':
            cur.execute("UPDATE AUTO SET AMOUNT_OF_FINES =  %s WHERE VIN_NUMBER_OF_AUTO = %s",
                        (request.form["amon"], vinn))
            con.commit()
        ...
        if year != request.form["yea"] and request.form["yea"] != '':
            cur.execute("UPDATE AUTO SET YEAR_OF_RELEASE =  %s WHERE VIN_NUMBER_OF_AUTO = %s",
                        (request.form["yea"], vinn))
            con.commit()
    ...
    return render_template('advred1.html', vinn=vinn, state=state, comm=comm, amo=amo, col=col, year=year,
                           mileage=mileage)
